...
<div id="video-filters" style="width: 100%; max-width: 800px; margin: 10px auto; padding: 15px; background: #f5f5f5; border-radius: 8px;">
    ...
    <div style="margin-bottom: 20px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px;">
            ...
            <div>
                <label>频闪减少: <span id="flicker-value">0%</span></label>
                <input type="range" id="flicker" min="0" max="100" value="0" style="width: 100%;">
            </div>
        </div>
    </div>
...
<script>
    ...
    function updateFilter() {
        ...
        // 频闪减少（通过轻微模糊和对比度调整）
        if (flicker > 0) {
            const flickerBlur = flicker * 0.02; // 轻微模糊
            const flickerContrast = 100 - flicker * 0.1; // 降低对比度
            filterString += ` blur(${flickerBlur}px)`;
            // 对比度调整会在SVG滤镜中处理
        }
            ...
            <svg style="position: absolute; width: 0; height: 0;">
                <defs>
                    <filter id="advanced-filter">
                        <!-- 频闪减少 - 时间平滑 -->
                        ${flicker > 0 ? `
                        <feGaussianBlur in="SourceGraphic" stdDeviation="${flicker * 0.01}" result="blurred"/>
                        <feBlend in="SourceGraphic" in2="blurred" mode="normal" result="antiFlicker"/>
                        ` : ''}
                        
                        <!-- 伽马校正 -->
                        <feComponentTransfer ${flicker > 0 ? 'in="antiFlicker"' : ''}>
                        ...
                        ${flicker > 0 ? `
                        <feComponentTransfer>
                            <feFuncR type="discrete" tableValues="${generateFlickerTable(flicker)}"/>
                            <feFuncG type="discrete" tableValues="${generateFlickerTable(flicker)}"/>
                            <feFuncB type="discrete" tableValues="${generateFlickerTable(flicker)}"/>
                        </feComponentTransfer>
                        ` : ''}
                    </filter>
                </defs>
    ...
    function applyRedFix() {
        // 针对胶片泛红的预设调整，默认采用：锐化 20% 红色通道 70% 绿色通道 106% 蓝色通道106% 对比度88% 饱和度 69% 色相3%，添加频闪减少15%
        const values = {
            brightness: 100,
            contrast: 85,
            saturate: 69,
    ...
    function applyOldFilmFix() {
        ...
        const values = {
            ...
            red: 95, green: 100, blue: 105, sharpen: 25, blur: 0.1, flicker: 20
        };
    ...
    function applyColorBalance() {
        ...
        const values = {
            ...
            red: 90, green: 100, blue: 110, sharpen: 10, blur: 0, flicker: 10
        };
